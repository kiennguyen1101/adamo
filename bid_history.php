<?php
  ...
  if ($item->count_contents($item_details)) {
    if ($_REQUEST['option'] == 'retract_bid' && $session->value('user_id') == $item_details['owner_id']) {
    ...
    $show_hidden_bid = ($item_details['owner_id'] == $session->value('user_id') || $session->value('adminarea') == 'Active') ? true : false;
