...
#################################################################
  ...
  if ($session->value('adminarea') != 'Active') {
    ...
    if (isset($_POST['form_save_settings'])) {
      ...
      $sql_update_site_tax = $db->query("UPDATE " . DB_PREFIX . "tax_settings SET site_tax = 1 WHERE tax_id=" . $_POST['site_tax']);
    }
    ...
    if ($_REQUEST['do'] == 'add_tax' || $_REQUEST['do'] == 'edit_tax') {
      ...
      $sql_select_all_countries_seller = $db->query("SELECT c.id, c.name, s.id AS state_id, s.name AS state_name, s.parent_id FROM
			...
			(c.id NOT IN (" . $selected_countries_seller . ") OR s.id NOT IN (" . $selected_countries_seller . ")) AND c.parent_id=0 ORDER BY c.country_order ASC, c.name ASC");
      ...
      $sql_select_all_countries = $db->query("SELECT c.id, c.name, s.id AS state_id, s.name AS state_name, s.parent_id FROM
			" . DB_PREFIX . "countries c LEFT JOIN " . DB_PREFIX . "countries s ON c.id=s.parent_id WHERE
			(c.id NOT IN (" . $selected_countries . ") OR s.id NOT IN (" . $selected_countries . ")) AND c.parent_id=0 ORDER BY c.country_order ASC, c.name ASC");
      ...
      foreach ($tax_user_types as $value) {
        // kien $box_checked = eregi($value[0], $row_tax['tax_user_types']) ? 'checked' : '';
