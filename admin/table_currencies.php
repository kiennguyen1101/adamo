...
else
{
	...
	if (isset($_POST['form_save_settings']))
	{
		...
		if (!empty($_POST['new_symbol']))
		{
			$sql_insert_currency = $db->query("INSERT INTO " . DB_PREFIX . "currencies
				(symbol, caption, convert_date) VALUES
				('" . $db->rem_special_chars($_POST['new_symbol']) . "', '" . $db->rem_special_chars($_POST['new_caption']) . "',
				" . CURRENT_TIME . ")");
		}
	...
	$sql_select_currencies = $db->query("SELECT id, symbol, caption, convert_rate, convert_date FROM
