<?php
  ...
  if ($session->value('adminarea') != 'Active') {
    ...
    $cat_details = $db->get_sql_row("SELECT * FROM " . DB_PREFIX . "categories WHERE
		category_id='" . intval($_GET['category_id']) . "'");

    $template->set('cat_details', $cat_details);

    $item_details = (!empty($_POST)) ? $_POST : $cat_details;
    $item_details['ad_image'][0] = (!empty($_POST)) ? $_POST['ad_image'][0] : $cat_details['image_path'];
    ...
    if (isset($_POST['form_submit_catopts'])) {
      ...
      $db->query("UPDATE " . DB_PREFIX . "categories SET 
			...
			image_path='" . $item_details['ad_image'][0] . "' WHERE category_id='" . $item_details['category_id'] . "'");
