  ...
  if ($session->value('adminarea') != 'Active') {
    ...
    if (isset($_REQUEST['form_save_settings'])) {
      ...
      if (count($_REQUEST['activate']) > 0) {
        $activate_array = $db->implode_array($_REQUEST['activate']); ## MyPHPAuction 2009 counter related routine
      ...
      if (count($_REQUEST['inactivate']) > 0) {
        $inactivate_array = $db->implode_array($_REQUEST['inactivate']); ## MyPHPAuction 2009 counter related routine
    ...
    $search_filter .= " WHERE w.creation_in_progress=0"; ## MyPHPAuction 2009 we always only show auctions with creation_in_progress = 0!
    if ($status == 'open') {
