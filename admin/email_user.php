  ...
  if ($session->value('adminarea') != 'Active') {
    ...
    if (isset($_POST['form_send_email'])) {
      ...
      if ($email_details['msg_method'] == 0) {
        send_mail($user_details['email'], $email_details['subject'], $email_details['email_content'], $setts['admin_email'], $db->add_special_chars($email_details['email_content']), null, true);
      }
      else {
        $msg = new messaging();
        $msg->setts = &$setts;
