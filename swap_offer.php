<?php
  ...
  if ($session->value('membersarea') != 'Active') {
    ...
    if (!$item_details['enable_swap'] || $session->is_set('swap_offer_id') || $session->value('user_id') == $item_details['owner_id'] || $blocked_user) {
      ...
      if (!empty($_POST['description'])) {## MyPHPAuction 2009 we will save the offer in the 'auction_offers' table
    ...
    (string) $swap_offer_page_content = null; ## MyPHPAuction 2009 now we display the page
