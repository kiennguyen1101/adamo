...
#################################################################
  ...
  if ($session->value('user_id')) {
    ...
    if ($banned_output['result']) {
      ...
      $user = new user();
      $user->setts = &$setts; ## MyPHPAuction 2009 now we will save all post variables selected
      $user->save_vars($_POST);

      $form_submitted = FALSE; ## MyPHPAuction 2009 if save button is pressed, proceed
      if ($_REQUEST['operation'] == 'submit') {
        ...
        if ($banned_output['result']) {
          ...
          $form_submitted = TRUE; ## MyPHPAuction 2009 atm we wont create any emails either until we decide how many ways of registration we have.
          (string) $register_success_message = null;

          $user_id = $user->insert($_POST);

          $template->set('register_success_header', header5(MSG_REGISTRATION_CONFIRMATION)); ## MyPHPAuction 2009 add signup fee procedure here.
          $signup_fee = new fees();
          ...
          if ($signup_result['amount']) {
            ...
            $register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE1 . '</p>'; ## MyPHPAuction 2009 include registration confirmation email
            ...
            // admin approval
            $sql_update_user = $db->query("UPDATE " . DB_PREFIX . "users SET
					active=1, approved=0, payment_status='confirmed', mail_activated=1 WHERE user_id=" . $user_id);

            $register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE2 . '</p>'; ## MyPHPAuction 2009 notify user & admin that user approval is required
            $mail_input_id = $user_id;
            ...
            $register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE0 . '</p>'; ## MyPHPAuction 2009 include registration success email
            $mail_input_id = $user_id;
