...
#################################################################
  ...
  if ($session->value('membersarea') != 'Active') {
    ...
    if (!show_makeoffer($item_details) || $session->is_set('make_offer_id') || $session->value('user_id') == $item_details['owner_id'] || $blocked_user) {
      ...
      if ($item->can_place_offer($item_details, $_POST['amount'])) {## MyPHPAuction 2009 we will save the offer in the 'auction_offers' table
    ...
    (string) $make_offer_page_content = null; ## MyPHPAuction 2009 now we display the page
