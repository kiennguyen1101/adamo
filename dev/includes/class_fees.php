...
class fees extends tax
{
	...
	function form_nganluong ($transaction_id, $nganluong_email, $nganluong_merchant_id, $nganluong_password, $payment_amount, $payment_description=null, $direct_payment=FALSE, $post_url='https://www.nganluong.vn/checkout.php')
	{
		...
		$display_output = '<table width="100%" border="0" cellspacing="2" cellpadding="3" class="paymenttable"> '.
			'<tr>'.
			'	<td width="160" class="paytable1"><img src="img/logo_nganluong.jpg"></td>'.
			'		<form method="post" action="' . $post_url . '" id="form_nganluong"> '.
			'	<td class="paytable2" width="100%">' . GMSG_NGANLUONG_DESCRIPTION . '</td> '.
			'	<td class="paytable3"> '.
			'		<input type="hidden" name="cmd" value="cart"> '.
			'		<input type="hidden" name="return_url" value="' . $this->return_url . '"> '.
			'		<input type="hidden" name="receiver" value="' . $nganluong_email . '"> '.
			'		<input type="hidden" name="transactionID" value="' . $transaction_id . '"> '.
			'		<input type="hidden" name="service_fee" value="' . GMSG_SERVICE_ACTIVATION_FEE . '"> '.
			'		<input type="hidden" name="price" value="' . $payment_amount . '"> '.
			'		<input type="hidden" name="transaction_info" value="' . $payment_description . '"> '.
			'		<input type="hidden" name="merchant_site_code" value="' . $nganluong_merchant_id . '"> '.
			'		<input type="hidden" name="secure_code" value="' . $secure_code . '"> '.
			'		<input type="hidden" name="url_process" value="' . $this->process_url . '">	'.
			'		<input name="submit" type="image" src="themes/' . $this->setts['default_theme'] . '/img/system/but_pay.gif" border="0"> '.
			'	</td></form> '.
			'</tr></table>';
	...
	*/
	...
	function show_gateways($transaction_id, $payment_amount, $currency, $user_id, $payment_description = null, $dp_gateways = null)
	{
		...
		while ($gateway_details = $this->fetch_array($sql_select_gateways))
		{
			...
			if ($gateway_details['name'] == 'nganluong' && (!$user_id || $pg_details['pg_nganluong_email']))
			{
				...
				$display_output .= $this->form_nganluong($transaction_id, $nganluong_email, $nganluong_merchant_id, $nganluong_password, $payment_amount,$payment_description);
			}
...
//        $sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices (user_id, name, amount, invoice_date, current_balance, live_fee, processor) VALUES ('" . $custom_id . "', '" . $invoice_name . "', '" . $payment_amount . "',	'" . CURRENT_TIME . "', '0', '1', '" . $payment_gateway . "')");
	...
	function auction_setup_fees($item_details, $user_details, $voucher_details = null, $apply_tax = true, $add_invoices = false, $item_relist = false)
	{
		...
		$output['display'] = 
		'<tr class="c4 fees_preview"> '.
      	'	<td align="right" style="font-weight: bold;font-size: 15px;">' . GMSG_AUCTION_FEES . '</td> '.
		'	<td>&nbsp;</td> '.
		'</tr> ';
