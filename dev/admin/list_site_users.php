...
#################################################################
  ...
  if ($session->value('adminarea') != 'Active') {
    ...
    if ($_REQUEST['do'] == 'user_details') {
      ...
      if ($approved) {## MyPHPAuction 2009 users counter - activate/inactivate all the user's auctions## MyPHPAuction 2009 but only if his account is approved - also activate/suspend the user's auctions
      ...
      $db->query("UPDATE " . DB_PREFIX . "users SET
			active='" . $value . "', approved='" . $approved . "', mail_activated=1 WHERE
			user_id=" . $user_id);
    }
