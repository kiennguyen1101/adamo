	...
	else
	{
      ...
      if (!$start)
		...
		if ($_REQUEST['do'] == 'edit_auction')
		{
			...
			$user_details = $db->get_sql_row("SELECT user_id, username, shop_account_id, shop_categories, shop_active,
				preferred_seller,	reg_date, country, state, zip_code, balance FROM " . DB_PREFIX . "users WHERE
				user_id=" . $item_details['owner_id']);## MyPHPAuction 2009 set here the checkboxes
			...
			if (isset($_REQUEST['form_edit_proceed']))
			{
				...
				else
				{
					...
					$template->set('msg_changes_saved', $msg_changes_saved);## MyPHPAuction 2009 item update function call
					
					$item->edit_auction = true;
					$item->insert($item_details, $user_details['user_id']);
				}
			}

			if (!$form_submitted)
			{## MyPHPAuction 2009 <<BEGIN>> media upload sequence
		...
		else if (isset($_REQUEST['form_save_settings']))
		{
			...
			{
				$approve_array = $db->implode_array($_REQUEST['approve']);## MyPHPAuction 2009 auctions counter - add process - multiple auctions (approve auctions)
			...
			{
				$activate_array = $db->implode_array($_REQUEST['activate']);## MyPHPAuction 2009 auctions counter - add process - multiple auctions (activate auctions)
			...
			{
				$inactivate_array = $db->implode_array($_REQUEST['inactivate']);## MyPHPAuction 2009 auctions counter - remove process - multiple auctions (inactivate auctions)
		...
		$search_filter .= " WHERE creation_in_progress=0 AND a.is_draft=0 ";## MyPHPAuction 2009 we always only show auctions with creation_in_progress = 0!
