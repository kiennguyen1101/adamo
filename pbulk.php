  ...
  function checkvar($type, $value) {
    ...
    switch ($type) {
      ...
      case "text":
        ...
        if (preg_match("/(select|insert|delete|drop).*(from|into)/is", $value))
          $value = "";
        if (strlen($value) > 1000000)
          $value = substr($value, 0, 1000000);
        break;
...
///////////////////  DO LOGIN CHECK   ////////////////////////
  ...
  $_SESSION['zip'] = $userinfo['zip'];
  $_SESSION['country'] = $userinfo['country'];

  if ($setts['private_site'] == "Y")
    $_SESSION['is_seller'] = $userinfo['is_seller'];
  else
...
///////////////  UPDATE ITEM FEATURES IN TMP TABLE ////////////////
  if ($_POST['lot']) {
    foreach ($_POST['lot'] as $key => $value) {
      if ($_POST['homefeat'][$value] == "on")
        $query[] = "homefeat='1'";
      else
        $query[] = "hpfeat='0'";
      if ($_POST['catfeat'][$value] == "on")
        $query[] = "catfeat='1'";
      else
        $query[] = "catfeat='0'";
      if ($_POST['highlight'][$value] == "on")
        $query[] = "highlight='1'";
      else
        $query[] = "highlight='0'";
      if ($_POST['bold'][$value] == "on")
        $query[] = "bold='1'";
      else
        $query[] = "bold='0'";
...
///////////////  PREPARE TO SUBMIT TO myphpauction AUCTIONS TABLE  ////////////////
  while ($row = mysql_fetch_array($result)) {
    if ($_POST['finished']) {
      ...
      ///////////////  INSERT INTO myphpauction AUCTIONS TABLE  ////////////////
