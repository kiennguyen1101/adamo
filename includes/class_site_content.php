...
class site_content extends database
{
    function subpage_title($page_handle)
    {
        ...
        switch ($page_handle) {
            case 'help':
                $output = AMSG_EDIT_HELP_SECTION;
                break;
            case 'news':
                $output = AMSG_EDIT_NEWS_SECTION;
                break;
            case 'faq':
                $output = AMSG_EDIT_FAQ_SECTION;
                break;
            case 'custom_page':
                $output = AMSG_CUSTOM_PAGES_MANAGEMENT;
                break;
            case 'about_us':
                $output = AMSG_EDIT_ABOUT_US_PAGE;
                break;
            case 'contact_us':
                $output = AMSG_EDIT_CONTACT_US_PAGE;
                break;
            case 'terms':
                $output = AMSG_EDIT_TERMS_PAGE;
                break;
            case 'privacy':
                $output = AMSG_EDIT_PRIVACY_PAGE;
                break;
            case 'announcements':
                $output = AMSG_EDIT_MEMBERS_ANNOUNCEMENTS;
                break;
            default:
                $output = GMSG_NA;
        }
    ...
    function insert_topic($variables_array, $lang, $page_id, $page_handle)
    {
        $result = $this->query("INSERT INTO " . DB_PREFIX . "content_pages
			(topic_name, topic_content, topic_lang, reg_date, page_id, page_handle) VALUES
			('" . $variables_array['topic_name_' . $lang] . "', '" . $variables_array['topic_content_' . $lang] . "',
			'" . $lang . "', '" . CURRENT_TIME . "', '" . $page_id . "', '" . $page_handle . "')");
    ...
    function edit_topic($variables_array, $lang, $page_id, $page_handle)
    {
        ...
        if ($is_topic) {
            $result = $this->query("UPDATE " . DB_PREFIX . "content_pages SET
				...
				topic_content='" . $variables_array['topic_content_' . $lang] . "' WHERE
				topic_lang='" . $lang . "' AND page_id='" . $page_id . "' AND page_handle='" . $page_handle . "'");
        } else {
