<?php
  ...
  class reputation extends item {
    ...
    function rep_rate($rate) {
      ...
      if ($rate <= 1)
        $display_output = '<img src="themes/' . $this->setts['default_theme'] . '/img/system/1stars.gif" border="0">';
      else if ($rate == 2)
        $display_output = '<img src="themes/' . $this->setts['default_theme'] . '/img/system/2stars.gif" border="0">';
      else if ($rate == 3)
        $display_output = '<img src="themes/' . $this->setts['default_theme'] . '/img/system/3stars.gif" border="0">';
      else if ($rate == 4)
        $display_output = '<img src="themes/' . $this->setts['default_theme'] . '/img/system/4stars.gif" border="0">';
      else if ($rate >= 5)
        $display_output = '<img src="themes/' . $this->setts['default_theme'] . '/img/system/5stars.gif" border="0">';
    ...
    function save($variables_array, $user_id) {
      ...
      $this->query("UPDATE " . DB_PREFIX . "reputation SET reputation_content='" . $variables_array['reputation_content'] . "',
			...
			submitted=1 WHERE reputation_id='" . $variables_array['reputation_id'] . "' AND from_id='" . $user_id . "'");

      $reputation_type = $this->get_sql_field("SELECT reputation_type FROM " . DB_PREFIX . "reputation WHERE 
			reputation_id='" . $variables_array['reputation_id'] . "'", 'reputation_type');

      $page_handle = 'reputation_' . $reputation_type;
      $this->update_page_data($variables_array['reputation_id'], $page_handle, $variables_array);
    }

    function calc_reputation($user_id, $reputation_type = null) {
