<?php
  ...
  require_once('class_CCache.php');

  class cache {

    static function createCache($cache) {

      /* Return the cache class on demand 
       * Default to apc cache.
       */
      switch (strtolower($cache)) {
        ...
        case 'memcache':
        case 'memcached':
          $memcached = new CMemCache();
          include_once('config.php');
          $memcached->setServers($memcache_servers);
          return $memcached;
          break;
        default:
          return new CApcCache();
    ...
    public function __construct($config) {
      if (is_array($config)) {
        foreach ($config as $key => $value)
          $this->$key = $value;
        if ($this->host === null)
          throw new Exception(printf('yii', 'CMemCache server configuration must have "host" value.'));
      }
      else
        throw new Exception(printf('yii', 'CMemCache server configuration must be an array.'));
    }
