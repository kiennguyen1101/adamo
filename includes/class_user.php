...
#################################################################
  ...
  class user extends custom_field {
    ...
    /* Constructor 
     * Call private function get to retrieve user's fields
     */
    ...
    /* Return associated array of fields from users table 
     * Will try to get user's data from cache. Set cache if not exists.
    ...
    /* magic method to get attribute(s) */
    ...
    /* magic method to set attribute(s) */

    public function __set($name, $value) {
      if ($name == 'attributes')
        $this->user_details = $value;
      $this->user_details[$name] = $value;
    }
    ...
    function insert($user_details, $page_handle = 'register') {
      ...
      $sql_insert_user = $this->query("INSERT INTO " . DB_PREFIX . "users
			...
			pg_ikobo_username, pg_ikobo_password, pg_nganluong_email, pg_nganluong_username, pg_nganluong_password, pg_protx_username, pg_protx_password, 
			...
			'" . $user_details['pg_ikobo_password'] . "', '" . $user_details['pg_nganluong_email'] . "',
			'" . $user_details['pg_nganluong_username'] . "','" . $user_details['pg_nganluong_password'] . "', 
			'" . $user_details['pg_protx_username'] . "','" . $user_details['pg_protx_password'] . "', 
			'" . $user_details['pg_authnet_username'] . "','" . $user_details['pg_authnet_password'] . "',
			'" . $user_details['pg_mb_email'] . "', '" . $user_details['pg_paymate_merchant_id'] . "', 
    ...
    function update($user_id, $user_details, $new_password = null, $page_handle = 'register', $admin_edit = false) {
      ...
      try {
        ...
        $sql_update_query = "UPDATE " . DB_PREFIX . "users SET
			...
			pg_nganluong_email = '" . $user_details['pg_nganluong_email'] . "',
			pg_nganluong_username = '" . $user_details['pg_nganluong_username'] . "',
			pg_nganluong_password = '" . $user_details['pg_nganluong_password'] . "',
        ...
        switch ($pg_details['name']) {
          ...
          case 'nganluong':
            $pg_settings_rows .= '<tr class="' . $background . '"> ' .
              ...
              '	<td><input name="pg_nganluong_email" type="text" value="' . $user_details['pg_nganluong_email'] . '" size="50"></td> ' .
              '</tr> ';
            $pg_settings_rows .= '<tr class="' . $background . '"> ' .
              '	<td width="250">' . GMSG_NGANLUONG_USERNAME . '</td> ' .
              '	<td><input name="pg_nganluong_username" type="text" value="' . $user_details['pg_nganluong_username'] . '" size="50"></td> ' .
              '</tr> ';
            $pg_settings_rows .= '<tr class="' . $background . '"> ' .
              '	<td width="250">' . GMSG_NGANLUONG_PASSWORD . '</td> ' .
              '	<td><input name="pg_nganluong_password" type="text" value="' . $user_details['pg_nganluong_password'] . '" size="50"></td> ' .
              '</tr> ';
            $pg_settings_rows .= '<tr> ' .
              '	<td></td> ' .
              '	<td class="' . $background . '">' . GMSG_NGANLUONG_CALLBACK . ':<br><br><b>' . SITE_PATH . 'pp_nganluong.php</b></td> ' .
              '</tr> ';
            break;
