...
class banner extends database
{
	function insert_banner($variables_array)
	{
		...
		$result = $this->query("INSERT INTO " . DB_PREFIX . "adverts
			...
			advert_categories, advert_code, advert_type) VALUES
			('" . $post_details['advert_url'] . "', '" . $variables_array['ad_image'][0] . "',
	...
	function edit_banner($variables_array, $advert_id)
	{
		...
		$result = $this->query("UPDATE " . DB_PREFIX . "adverts SET
			advert_url='" . $post_details['advert_url'] . "', advert_img_path='" . $variables_array['ad_image'][0] . "',
			...
			advert_type='" . $post_details['advert_type'] . "'	WHERE	advert_id='" . $advert_id . "'");
	...
	function select_banner($base_url, $parent_id, $auction_id)
	{
		(string) $display_output = null;
		...
		$advert_query = "WHERE (views_purchased=0 OR views_purchased>=views) AND
			(clicks_purchased=0 OR clicks_purchased>=clicks)";
