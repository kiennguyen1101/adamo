...
class voucher extends database
{
	...
	function voucher_settings ($variables_array)
	{
		...
		$output['voucher_code'] = substr(md5(uniqid(rand(2, 999999999))),-12);
	...
	function add_voucher($variables_array)
	{
		...
		$sql_insert_voucher = $this->query("INSERT INTO " . DB_PREFIX . "vouchers
			...
			assigned_fees, voucher_name, voucher_duration) VALUES
			('" . $voucher_settings['voucher_code'] . "', '" . $variables_array['voucher_type'] . "',
			...
			'" . $variables_array['voucher_duration'] . "')");
	}

	function edit_voucher($variables_array)
	{
		$variables_array = $this->rem_special_chars_array($variables_array);
		...
		$sql_update_voucher = $this->query("UPDATE " . DB_PREFIX . "vouchers SET
			...
			voucher_duration='" . $variables_array['voucher_duration'] . "' WHERE voucher_id = '" . $variables_array['voucher_id'] . "'");
	}
