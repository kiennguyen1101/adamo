...
#################################################################
  ...
  if (isset($_REQUEST['form_convert'])) {
    $currency_from_value = $db->get_sql_field("SELECT convert_rate FROM " . DB_PREFIX . "currencies WHERE symbol='" . $_REQUEST['currency_from'] . "'", 'convert_rate');
    $currency_to_value = $db->get_sql_field("SELECT convert_rate FROM " . DB_PREFIX . "currencies WHERE symbol='" . $_REQUEST['currency_to'] . "'", 'convert_rate');
