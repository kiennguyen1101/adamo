...
<?php echo $categories_initialize_msg; ?>
// -->
</SCRIPT>
<SCRIPT language=javascript>
<!--
...
//arrayLevel 0 for top level, 1 for next level down etc...
  function populate(arrayLevel)
  {
    ...
    {
<?php echo $category_id_type; ?>.value = "";	//clear any value from the category box
      ...
      {
<?php echo $category_id_type; ?>.value = strOptionValue;
      }
...
//arrayLevel 0 for top level, 1 for next level down etc...
  function prePopulate(mCat)
  {
    //check mCat is sane
    if (mCat.length < 5)
      return;					//must be at least 2 levels
    if (mCat.length % 5 != 0)
      return;				//each mCat is 5 chars long
    if (mCat.charAt(mCat.length - 1) != '-')
      return;//last char of last mCat is '-'
...
<?php echo $category_id_type; ?>.value = intMcat;
...
<?php echo $category_id_type; ?>.value = docSelectorArray[intArrayLevel].options[intArrayOption].value;
  ...
  <?php if ($current_step == 'addl_category') { ?>  
      <tr class="contentfont">
        <td class="c1" align="right" width="100%"><table cellspacing="2" border="0">
            <tr>
              <td><input id="form_next_step" type=submit value="<?php echo MSG_SKIP_THIS_STEP; ?>" name="form_next_step">
              </td>
            </tr>
          </table></td>
      </tr>
...
<td class="c1" align="right"><input id="form_next_step" type=submit value="<?php echo GMSG_NEXT_STEP; ?>" name="form_next_step"></td>
</tr>
</table>
<SCRIPT language=javascript>
<!--
  ...
  for (i = 0; i < document.ad_create_form.elements.length; i++)
  {
    ...
    {
<?php echo $category_id_type; ?> = document.ad_create_form.elements[i];
    }
