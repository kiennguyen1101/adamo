...
## Set your version
  ...
  if ($request == "config") {
    ...
    if ($userinfo = mysql_fetch_array($result)) {
      ...
      if ($num_durations) {
        while ($myrow = mysql_fetch_row($getdurations))
          echo "<PDUR>$myrow[1]\n";
      }
      ...
      if ($num_currencies) {
        while ($myrow = mysql_fetch_row($getcurrencies))
          echo "<PCURR>$myrow[1]\n";
      }
      ...
      if ($num_countries) {
        while ($myrow = mysql_fetch_row($getcountries))
          echo "<PLOC>$myrow[1]$sep$myrow[2]$sep$myrow[3]$sep\n";
      }
      ...
      if ($num_shipoptions) {
        while ($myrow = mysql_fetch_row($getshipoptions))
          echo "<PSHIP>$myrow[1]\n";
      }
